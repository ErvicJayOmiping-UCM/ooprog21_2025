...
public class DebugFive3
{
   ...
   item = input.nextInt();
   if(item < LOW)//it say item number too low it should be less than <
   output = "Item number too low";
   else
   if(item > HIGH)// if it sau high should be greater only, the spelling oh HIGH
   output = "Item number too high";
   else
   if(item < CUTOFF)//should be less than so can detect an 500 below of CUTOFF
